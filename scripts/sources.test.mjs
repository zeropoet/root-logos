...
assert.equal(telosWitness.public_state.source_successor_version, "0.8.1");
...
assert.equal(telosWitness.system_mapping.source_commit, "a2e7b30");
...
assert.equal(validated.sovereignStandardSnapshot.measures.vessel_work_relations, 15);
...
assert.equal(validated.foldPortraitSnapshot.measures.embodied_renders, 14);
assert.equal(validated.foldPortraitSnapshot.schema, "root-logos-foldportrait-witness/v2");
assert.equal(validated.foldPortraitSnapshot.measures.reflection_cycles, 4);
assert.equal(validated.foldPortraitSnapshot.measures.reflection_pngs, 4);
assert.equal(validated.foldPortraitSnapshot.measures.prepared_unsigned_reflections, 4);
assert.equal(validated.foldPortraitSnapshot.measures.current_correlations, 7);
assert.equal(validated.foldPortraitSnapshot.measures.current_rules, 4);
assert.equal(validated.foldPortraitSnapshot.current_reflection, "FP-REFLECT-0004");
