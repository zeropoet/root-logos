...
assert.equal(refreshed.portrait.measures.represented_works, 108);
assert.equal(refreshed.portrait.measures.canonical_supply_ceiling, 108);
assert.equal(refreshed.portrait.measures.remaining_capacity, 0);
...
assert.equal(telosWitness.public_state.current_version, "1.14.1");
assert.equal(telosWitness.public_state.studio_surfaces_live, 8);
assert.equal(telosWitness.public_state.telos_managed_deployments_aligned, 5);
assert.equal(telosWitness.public_state.source_successor_version, "1.14.1");
assert.equal(telosWitness.public_state.deployed_release_version, "1.14.1");
...
assert.equal(telosWitness.system_mapping.repositories.length, 8);
...
assert.equal(telosWitness.system_mapping.source_commit, "9a78438f729653eb90c27fd8a42be6d2802295b6");
assert.ok(telosWitness.system_mapping.repositories.some(({ id }) => id === "ovel"));
assert.ok(telosWitness.system_mapping.repositories.some(({ id }) => id === "the-record"));
assert.ok(telosWitness.system_mapping.relations.includes("Telos surface observer -> verifies without altering -> eight canonical studio surfaces"));
...
assert.equal(validated.sovereignStandardSnapshot.measures.witness_works, 109);
